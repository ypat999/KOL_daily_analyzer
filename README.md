...
1. 从B站获取固定几个UP主的限定时间内最新视频（周末运行时，只收录周五收盘后发布的内容），提取字幕，交由DeepSeek分别总结
2. 获取固定几个公众号的限定时间内最新文章（周末运行时，只收录周五收盘后发布的内容），交由DeepSeek分别总结
...
### 注意事项
- 脚本会自动过滤限定时间内发布的文章（周末运行时，只收录周五收盘后发布的内容）
- 包含反爬虫机制，添加了随机延迟
- 支持断点续传，已保存的文章不会重复处理
...
### 注意事项
- 脚本会自动过滤限定时间内发布的视频（周末运行时，只收录周五收盘后发布的内容）
